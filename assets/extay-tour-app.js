(() => {
  ...
  const t = (value) => value && typeof value === 'object' && !Array.isArray(value)
    ...
    : value;
  const labels = () => ({
    ko: { places: '곳', from: 'EXTAY에서', map: '지도', official: '상세 정보', empty: '조건에 맞는 장소가 없습니다.' },
    en: { places: 'places', from: 'FROM EXTAY', map: 'MAPS', official: 'DETAILS', empty: 'No places match your search.' },
    ja: { places: 'スポット', from: 'EXTAYから', map: '地図', official: '詳細情報', empty: '条件に合うスポットがありません。' },
    zh: { places: '处', from: '从EXTAY', map: '地图', official: '详细信息', empty: '没有符合条件的地点。' }, "zh-TW":{ places: "處", from: "從EXTAY", map: "地圖", official: "詳細資訊", empty: "沒有符合條件的地點。" },
  }[lang()] || {});
